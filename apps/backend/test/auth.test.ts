...
} from "../src/owner-registration.ts";

describe("Entity Library owner registration policy", () => {
  test("requires a successfully verified first passkey and keeps one stable owner", () => {
    expect(
      ownerRegistrationStatus({ ownerExists: false, passkeyExists: false }),
    ).toEqual({ ownerRegistered: false });
    expect(ownerRegistrationUser({ ownerExists: false }).id).toBe(
      OWNER_USER_ID,
    );
    expect(
      authorizeOwnerPasskeyRegistration({
  ...
  test("blocks new ownership after registration and permits only the owner to add passkeys", () => {
    expect(() => ownerRegistrationUser({ ownerExists: true })).toThrow(
  ...
  test("fails closed on incomplete restored ownership", () => {
    expect(() =>
      ownerRegistrationStatus({ ownerExists: true, passkeyExists: false }),
    ).toThrow("invalid");
    expect(() =>
      ownerRegistrationStatus({ ownerExists: false, passkeyExists: true }),
    ).toThrow("invalid");
  });
});
