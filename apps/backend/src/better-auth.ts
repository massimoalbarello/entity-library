...
const PASSKEY_RELYING_PARTY_NAME = "Entity Library";
...
export function createAuthOptions({
  ...
  return {
    database: bunSqlAdapter({
      sql: database,
      tablesPrefix: BETTER_AUTH_TABLES_PREFIX,
    }),
      ...
      passkey({
        ...
        registration: {
          ...
          resolveUser: async ({ ctx }) => {
            try {
              const owner =
                await ctx.context.internalAdapter.findUserById(OWNER_USER_ID);
          ...
          afterVerification: async ({ ctx, verification, user }) => {
            try {
              if (user.id !== OWNER_USER_ID) {
                throw new OwnerRegistrationError(
